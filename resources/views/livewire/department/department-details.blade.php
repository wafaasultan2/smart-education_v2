    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-4">
                    <h3 class="title mb-2">أسم القسم</h3>
                    <span class="ps-4 text-secondary">{{ $department?->name ?? 'غير محدد' }}</span>
                </div>

                <!-- Section: Department Plan -->
                <hr>
                <h3 class="title mb-3">تفاصيل خطة القسم</h2>
                    <div class="mb-4 ps-4">
                        <h4 class="title mb-2">الخطه</h4>
                        <span class="ps-4 text-secondary">{{ $department?->plans?->first()->name ?? 'غير محدد' }}</span>

                        <h4 class="title mb-2">الوصف</h4>
                        <p class="text-muted ps-4">{{ $department?->plans?->first()->description ?? 'غير محدد' }}</p>
                    </div>

                    <!-- Section: Hourly Rates -->
                    <hr>
                    <h3 class="title mb-4">أسعار الساعات</h3>
                    <div class="row g-3">
                        @foreach (\App\Enums\AcademicDegree::cases() as $degree)
                            <div class="col-lg-4 col-md-6 col-sm-12">
                                ...
                                <span class="text-secondary">السعر الحالي:
                                    {{ $department?->{$degree->value} ?? 'غير محدد' }}</span>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-muted">{{ $department?->description ?? 'غير محدد' }}</p>
                    </div>
            </div>
        </div>
    </div>
