<tr>
    <td>{{ $course->id }}</td>
    <td>{{ $course->name }}</td>
    <td>{{ \App\Enums\Levels::{$course->level}->getValue() }}</td>
    <td>{{ \App\Enums\Terms::{$course->term}->getValue() }}</td>
    <td>{{ \App\Enums\CourseType::{$course->type}->getValue() }}</td>
    <td>
        @foreach ($course->plans as $plan)
            {{ $plan->name }} - {{ $plan->department->name }}<br>
        @endforeach
    </td>

    <td>
        <svg xmlns="http://www.w3.org/2000/svg"  onclick="showDetails({{ $course->id }}, 'showCourse')" width="24"
        ...
        <svg xmlns="http://www.w3.org/2000/svg"
            onclick="customConfirm({{ $course->id }}, '{{ $course->name }}', 'courseDelete')"
